...
export default function PantallaRegistros() {
    ...
    const fetchRegistros = async () => {
        try {
            ...
            setLoading(true);
            setError('');
            const response = await axios.get<Registro[]>(`${IPS.SERVER_URL}${IPS.API.REGISTRO_URL}/get`);
    ...
    const getButtonGradientColors = (isDelete = false) => {
        if (isDelete) {
            return isDarkMode
    ...
    // Componente de header para la FlatList
            ...
            <View style={localStyles.headerSection}>
                ...
                <View style={localStyles.headerControls}>
                    {registros.length > 0 && (
                        ...
                        >
                            {eliminando ? (
                                <ActivityIndicator size="small" color={isDarkMode ? "#FC8181" : "#E53E3E"} />
                            ) : (
                                <MaterialIcons
                                    name="delete"
                                    size={22}
    ...
    return (
        <SafeAreaView style={baseStyles.screen}>
            ...
            {loading && !refreshing ? (
                // Vista de carga separada para evitar problemas con FlatList
                <LoadingView />
            ) : (
                // Usamos la FlatList directamente sin envolverla en un ScrollView
                <FlatList
                    data={registros}
