...
export default function PantallaPuerta() {
    ...
    const obtenerEstadoRealPuerta = async () => {
        try {
            ...
            const response = await axios.get<{ status: string }>('http://192.168.8.4/api/arduino/doorstatus');
    ...
    const handleTogglePuerta = async () => {
        try {
            ...
            const url = puertaAbierta
                ? 'http://192.168.8.5:8082/api/door/cerrar'
                : 'http://192.168.8.5:8082/api/door/abrir';
