...
export default function PantallaRegistroUsuarios() {
    ...
    const loadSubUsers = async () => {
        try {
            ...
            const response = await axios.get<SubUsersResponse>('http://192.168.8.2:8082/api/subusers', {
    ...
    const handleRegisterUser = async () => {
        ...
        try {
            ...
            const response = await axios.post<SubUserResponse>(
                'http://192.168.8.2:8082/api/subusers/register',
                {
                    name,
                    accessMethod,
                    accessId
                    ...
                    onPress: async () => {
                        try {
                            ...
                            if (accessMethod === 'rfid') {
                                try {
                                    await axios.delete(
                                        `http://192.168.8.2:8082/api/rfids/${accessId}`,
                                        { headers: { Authorization: `Bearer ${token}` } }
                                    );
                            ...
                            const deleteResponse = await axios.delete(
                                `http://192.168.8.2:8082/api/subusers/${id}`,
                                { headers: { Authorization: `Bearer ${token}` } }
                            );
            ...
            if (accessMethod === 'rfid') {
                try {
                    ...
                    const checkResponse = await axios.post(
                        'http://192.168.8.2:8082/api/rfids/check',
                    ...
                    // Registrar el RFID
                    await axios.post(
                        'http://192.168.8.2:8082/api/rfids/register',
                        {
    ...
    const handleRegister = async () => {
        try {
            ...
            const response = await axios.post<RegisterResponse>('http://192.168.8.2:8082/api/users/register', {
                // datos de registro
            });
